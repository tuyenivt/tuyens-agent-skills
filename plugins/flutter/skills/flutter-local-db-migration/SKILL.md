...
> Load `Use skill: stack-detect` first to determine the project stack. Store selection lives in `flutter-data-persistence`; this skill owns schema versioning and the upgrade path. Compose `Use skill: ops-backward-compatibility` - old app versions stay installed on user devices for months, so the schema a shipped build understands is a compatibility surface.

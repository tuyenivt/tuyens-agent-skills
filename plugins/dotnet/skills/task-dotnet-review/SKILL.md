...
For each extra scope, spawn one independent subagent **in parallel** with the main thread. Use the **declared subagent for that scope** (`subagent_type` below) - do not infer the agent from the scope name; an observability review is not a `dotnet-tech-lead` spawn:

| Scope | Skill                              | Subagent (`subagent_type`)      |
| ----- | ---------------------------------- | ------------------------------- |
| +Perf | `task-dotnet-review-perf`          | `dotnet-performance-engineer`   |
| +Sec  | `task-dotnet-review-security`      | `dotnet-security-engineer`      |
| +Obs  | `task-dotnet-review-observability` | `dotnet-observability-engineer` |

...
- [ ] **Response-DTO field-stripping audit (when a response DTO IS used)**: even when the action returns a `record OrderResponse(...)`, audit the DTO definition for fields that should never reach the client. Common offenders: `PasswordHash` / `EncryptedPassword`, `MfaSecret` / `OtpSecret` / `RecoveryCodes`, `ApiKey` / `WebhookSecret`, `InternalNotes` / `AdminNotes` / `AuditLog`, `IsAdmin` / `Role` / `InternalRole`, `TenantInternalFlags`, `DeletedAt`, `InternalCreatedBy`, `LastLoginIp`. Two valid stances: (a) strip via `[JsonIgnore]` on the entity property when the entity is reused as the response (smell - prefer separate types), or (b) define a separate `OrderResponse` record naming only the public fields. `[JsonIgnore]` on the entity is fragile - a future engineer adding the field forgets the attribute. Separate response DTO is preferred. `[JsonInclude]` on private setters is also a leak surface to check
- [ ] **HTTP `Idempotency-Key` header on retry-prone POSTs (distinct from worker-side message dedup)**: client-driven retries on `POST /payments`, `POST /orders`, `POST /refunds` need server-side replay protection - a `request_idempotency` table keyed by `(tenant_id, idempotency_key)` storing the request hash + cached response. The first request runs the handler; subsequent requests with the same key return the cached response. This is **distinct** from worker-side message dedup (which protects against broker re-delivery): HTTP idempotency protects against client retries, worker dedup protects against broker re-delivery. A change that adds worker dedup but no HTTP idempotency on a retry-prone endpoint still has a double-charge surface visible to the client. Flag missing `Idempotency-Key` handling on POST endpoints that mutate billing / payment / refund state
...
- [ ] **Transaction boundaries**: writes spanning multiple use cases or multiple `DbContext` instances run inside an explicit `IDbContextTransaction` via the **async API** (`await using var tx = await db.Database.BeginTransactionAsync(ct); ...; await tx.CommitAsync(ct);`). The sync overloads `db.Database.BeginTransaction()` / `tx.Commit()` / `tx.Rollback()` on the request path are findings - they block the calling thread the same way `.Result` does. For single-`SaveChangesAsync` use cases, EF Core's implicit transaction suffices
- [ ] **No hardcoded JWT signing key in source**: any `new SymmetricSecurityKey(Encoding.UTF8.GetBytes("literal"))` or string-literal `IssuerSigningKey` in `Program.cs` / `Startup.cs` is `[Blocker]` - anyone with read access to the repo can forge tokens. Sourced from env / Vault / `dotnet user-secrets` (dev) only. This check lives in Core (not just the security subagent) because `core-only` reviews must still catch it

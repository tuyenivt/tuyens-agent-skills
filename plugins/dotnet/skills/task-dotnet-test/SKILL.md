...
- [ ] **SDK / non-default-transport bypass surfaces verified**: `WireMock.Net` / `MockHttp` only intercept calls that go through the SDK's default `HttpClient` pipeline. SDKs and clients that bypass this need explicit per-SDK redirection - a test that "looks stubbed" but quietly hits the real network is the worst failure mode. Verify each surface present in the diff:
  - **`AWS SDK for .NET` (`AWSSDK.*`)**: requires `clientConfig.ServiceURL = "http://localhost:4566"` (LocalStack) **and** `clientConfig.UseHttp = true` plus `clientConfig.AuthenticationRegion = "us-east-1"`. Default credentials chain hits IMDS / env / shared file - inject `BasicAWSCredentials` explicitly in tests. `AmazonS3Client` ignores `HttpClient` factory overrides for signing
  - **Google Cloud SDKs (`Google.Cloud.*`)**: storage emulator needs `STORAGE_EMULATOR_HOST=localhost:9023`; Pub/Sub needs `PUBSUB_EMULATOR_HOST`; Datastore needs `DATASTORE_EMULATOR_HOST`. Without these env vars the SDK reaches the real API even when `WireMock` is running
  - **gRPC via `Grpc.Net.Client`**: HTTP/2 is required - `WireMock.Net` / `mockttp` cannot serve HTTP/2 cleartext (h2c) by default. Stub via `Grpc.AspNetCore` test server or `tonic`-style `TestServerCallContext`; for full integration use a real `WebApplicationFactory<TStartup>` exposing the gRPC service over the test HTTP/2 server
  - **Custom `HttpClientHandler` / `SocketsHttpHandler` / `IHttpClientFactory` typed clients**: when production code does `new HttpClient(new SocketsHttpHandler { Proxy = ..., ServerCertificateCustomValidationCallback = ... })`, the test must redirect via `WebApplicationFactory.WithWebHostBuilder(b => b.ConfigureServices(s => s.AddHttpClient<TClient>().ConfigurePrimaryHttpMessageHandler(() => stubHandler)))` - replacing the default `IHttpClientFactory` registration. A `WireMock` server alone does not catch this if the typed client has a custom primary handler
  - **`SmtpClient`, `Microsoft.Data.SqlClient`, `Npgsql`**: not HTTP - need their own test-doubles (`Papercut-SMTP` Testcontainer for mail, real Testcontainers DB for SQL)
  - **Verification rule**: every stub must assert call count > 0 (e.g., `wireMock.LogEntries.Should().NotBeEmpty();` or `mockHttp.GetMatchCount(req).Should().BeGreaterThan(0);`). A passing test with zero recorded interceptor hits is the silent-bypass signature

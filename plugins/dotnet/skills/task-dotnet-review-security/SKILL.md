...
- [ ] **JWT validation parameters all `true`**: `ValidateIssuer = true`, `ValidateAudience = true`, `ValidateLifetime = true`, `ValidateIssuerSigningKey = true`. Set `ValidIssuer`, `ValidAudience`, `IssuerSigningKey` explicitly. `ClockSkew = TimeSpan.FromSeconds(30)` (default is 5 minutes - tightens token expiry precision)

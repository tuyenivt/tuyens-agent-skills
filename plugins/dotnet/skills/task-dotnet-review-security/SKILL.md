...
Use skill: `dotnet-security-patterns` for canonical .NET security patterns (auth middleware, JWT validation, FluentValidation, Argon2 / BCrypt / `IPasswordHasher`, CORS, secrets management, `dotnet list package --vulnerable`).

This step is a **triage pass**, not a separate findings list. Mark each OWASP category `yes` (signal present in diff) or `no signal in diff`; Steps 5-9 produce the actual findings.

| Risk                          | .NET-specific signal in diff                                                                                                                                       |
| ----------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Broken Access Control         | New action without explicit `[Authorize]` / `[AllowAnonymous]`; missing ownership check in handler/service body                                                     |
| Injection                     | `FromSqlRaw($"...{input}")` / `ExecuteSqlRaw($"...{input}")`; Dapper `QueryAsync<T>($"...{input}")`; `Process.Start("cmd.exe", $"/c {input}")` / `bash -c` shell-out |
| Cryptographic Failures        | Hardcoded JWT signing key (`SymmetricSecurityKey(Encoding.UTF8.GetBytes("literal"))`); `MD5` / `SHA1` / raw `SHA256` for passwords; `Random` for tokens/nonces      |
| Security Misconfiguration     | Missing security headers (HSTS / X-Frame-Options); `AllowAnyOrigin().AllowCredentials()` near-miss via `SetIsOriginAllowed(_ => true)`; `UseDeveloperExceptionPage()` in prod; Hangfire / Swagger UI exposed without auth |
| SSRF                          | `HttpClient.GetAsync(userControlledUrl)` without hostname allowlist (no RFC1918 / link-local / cloud-metadata rejection)                                            |
| XSS                           | `@Html.Raw(...)` on user content; `Response.WriteAsync($"<div>{user}</div>")` manual HTML interpolation                                                             |
| Insecure Design (A04)         | New controllers default-allow rather than default-deny via `FallbackPolicy = RequireAuthenticatedUser()` with explicit `[AllowAnonymous]` opt-outs                  |
| Vulnerable Components (A06)   | `Cargo.lock` / `packages.lock.json` change with stale CVE; `dotnet list package --vulnerable` not in CI; `<NuGetAudit>true</NuGetAudit>` (.NET 8+) not enabled      |
| Data Integrity Failures (A08) | `[FromBody] DomainEntity` / `JsonSerializer.Deserialize<DomainEntity>(body)` (mass assignment); `BinaryFormatter.Deserialize` reintroduced; `Newtonsoft.Json` `TypeNameHandling.All` / `Auto` / `Objects`; `XmlSerializer` without `DtdProcessing.Prohibit`; `unsafe` use |
| Logging & Monitoring (A09)    | Serilog / `ILogger` logs `password` / `token` / `Authorization` / `Cookie`; missing auth-event logging; Sentry / Application Insights without PII-stripping `BeforeSend` |

...
Opt-in plugin that builds a **persistent codebase knowledge graph** at `.codemap/graph.json`, plus a small family of workflows that read from it: ask the graph questions, play guided walkthroughs, deep-dive on a single entity. The graph is refreshed on demand by running `/task-codemap` - there is no automatic per-commit sync; teams typically rebuild on a schedule (e.g. weekly on the main branch) so the graph reflects a whole-team view rather than one person's local edits.
...
| Skill                   | Description                                                                                                                                                                                                                                                                                                                                                              |
| ----------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `task-codemap`          | **Build or sync the graph.** Auto-detects mode: first run does a full build (scan -> batch -> parallel sub-agent analyze -> merge -> layer -> guide -> validate); subsequent runs do an incremental sync via fingerprint diff, re-analyzing only changed files. Use `--full` to force a rebuild.                                                                            |
...
**Gitignore:** `.codemap/intermediate/`.

**Keep it fresh:** run `/task-codemap` after pulls and before releases. There is no auto-update hook - refresh is deliberate. The recommended cadence is a scheduled rebuild on the main branch (e.g. weekly), so one whole-team refresh replaces many divergent per-developer syncs.

For graphs above ~10 MB, track with **git-lfs**.

**Note on diff churn:** `meta.json#builtAt` and `fingerprints.json#computedAt` are ISO timestamps, so every rebuild produces a non-zero diff even when the graph itself is unchanged. With a weekly main-branch rebuild cadence this churn surfaces only once per cycle. If the diff noise is still a problem, consider a pre-commit hook that strips those timestamps, or accept the churn in exchange for a recorded build time.

...
| SSRF                       | `WebClient` / `RestClient` validates hostnames against an allowlist. Validator rejects private / loopback ranges (10/8, 172.16/12, 192.168/16, 169.254/16, 127/8, ::1, fc00::/7) **after DNS resolution** (DNS-rebinding defense) - resolve once + pin IP, or `ClientHttpRequestInterceptor` re-checks. Hostname-string allowlists alone are insufficient. |

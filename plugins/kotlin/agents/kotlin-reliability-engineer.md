...
| Make it faster under normal load (N+1, dispatcher tuning, cache hit ratio) | `kotlin-performance-engineer` - this agent owns behavior under failure and saturation, not throughput; a bare slowness report routes to perf unless the fix is bounding / shedding at saturation, which stays here |
...
| Define SLIs / SLOs, error budgets, what to alert on | `kotlin-observability-engineer` owns SLI / SLO definition; this agent supplies the mechanisms those targets measure |

A bundled ask (slices owned by different rows) splits per this table; multiple findings all in this agent's scope are one review pass, not a split. The reliability slice runs here first - the mechanism must exist before `kotlin-observability-engineer` reviews its visibility; other slices sequence independently after the split.

---
name: task-go-review-api
description: Go API-contract review - REST design, breaking-change/versioning, response DTOs over GORM entities, RFC 9457 errors, pagination, OpenAPI drift.
agent: go-api-engineer
metadata:
  category: backend
  tags: [go, gin, api, rest, contract, compatibility, versioning, openapi, workflow]
  type: workflow
user-invocable: true
---
...
Go-aware API-contract review naming Gin routes and binding structs, GORM-entity-to-response-DTO boundaries, breaking-change detection from the consumer's view, `/v1/` versioning and `Sunset` deprecation, RFC 9457 error envelopes, pagination shape, and swaggo / OpenAPI drift. API review = the contract the service exposes: its REST shape, its evolution across versions, and its honesty about what it returns. Every finding names who breaks and how, with concrete fixes for Go 1.25+ / Gin.

Stack-specific delegate of `task-code-review-api` for Go.
...
- **vs. core Phase B:** `task-go-review` Phase B owns the handler returning the right data; this lens owns that data's shape being conventional, versioned, and stable. Raw-GORM-entity-vs-response-struct leakage sits at the seam - the umbrella dedups.
...
| `deep`     | Requested, or handed down by `task-go-review`   | All + `Consumer-Impact Map`                   |
...
| Form | Meaning |
|------|---------|
| `/task-go-review-api` | Current branch vs base; fails fast on trunk |
| `/task-go-review-api <branch>` | `<branch>` vs base (3-dot) |
| `/task-go-review-api pr-<N>` | PR head fetched into local branch (user runs fetch) |

Append `deep` to request the deep pass (e.g. `/task-go-review-api <branch> deep`); `standard` is accepted explicitly. A `--base <branch>` argument (forwarded by `task-code-review-api`) passes to `review-precondition-check` as the explicit base override. When invoked as subagent (e.g. by `task-go-review`), the parent passes the pre-confirmed stack and precondition handle + pre-read diff; Steps 2-3 consume those instead of re-running, and Step 9 returns findings instead of writing - the parent owns the report.
...
- Route registration: Gin `router.Group` / `GET` / `POST` / `PUT` / `PATCH` / `DELETE` wiring - paths, methods, version prefixes
- Request binding structs and their `binding:` / `json:` tags - required fields, constraints, field names on the wire
- Response DTOs / structs returned via `c.JSON` - and whether any handler returns a GORM entity directly
- Error handling on the response path - `c.JSON(status, ...)` shapes, error middleware, status-code mapping
- Pagination: collection handlers and their query params / response envelope
- swaggo annotations / committed OpenAPI spec (if Step 2 found one)
...
- [ ] **Response changes judged from the consumer** - a removed, renamed, or retyped response field; a status-code change; a new enum value in a response field; or a changed error shape is **breaking** until a search proves no consumer. Added optional fields are safe. Enum widening resolves by verify-then-emit: when the search proves every consumer tolerates unknown values and the spec's enum is updated, it is compatible; emitting before that proof is a High finding.
- [ ] **Request changes judged from the consumer** - a new **required** request field, a tightened `binding` constraint (`required`, `min`, `oneof`), or a removed accepted field breaks existing callers. New optional fields are safe.
...
Use skill: `backend-api-guidelines` for the design rules. Use skill: `go-gin-patterns` for Gin routing and response conventions.
...
Use skill: `go-gin-patterns` for handler and binding conventions, and `go-data-access` for keyset-vs-offset guidance at the data layer. Where their internal-facing examples differ from this workflow's contract rules (they show a generic `data`/`meta` success envelope, a non-9457 error shape, and page/offset pagination), **this workflow's rules win** - it reviews the published contract; they build internals.

- [ ] **Response DTO, never a raw entity** - handlers return a dedicated response struct, not a `gorm.Model` / DB entity. Returning the entity over-exposes internal fields (`PasswordHash`, internal FKs, soft-delete timestamps) and couples the wire contract to the DB schema, so a column rename silently breaks clients.
- [ ] **RFC 9457 error envelope** - errors return `type` / `title` / `status` / `detail` / `instance`, consistently across handlers. No Go stack trace, no `gorm.ErrRecordNotFound` string, no internal ID leaked to the client.
- [ ] **Every collection paginated** - a list endpoint returns a bounded page with a consistent envelope (`items` + `next_cursor` / total), never an unbounded `Find(&all)`. Cursor for large / write-heavy collections, offset only for small / stable ones.
- [ ] **Field naming and nullability consistent** - `json:` tags follow one convention (snake_case or camelCase, not mixed); a field that can be absent is `omitempty` or documented nullable, not a silent zero-value.
...
- [ ] **Idempotency-Key in the contract** - a non-idempotent `POST` on money / notification / provisioning declares an `Idempotency-Key` header in its documented request (the dedup *correctness* routes to `task-go-review-reliability`).
- [ ] **OpenAPI matches code** _(only if Step 2 found a published spec)_ - changed routes, DTO fields, status codes, and error shapes are reflected in the swaggo annotations / spec. A field in the struct but absent from the annotations (or a documented field the handler no longer returns) is drift, and generated clients will be wrong.

```go
// Bad - returns the GORM entity: leaks PasswordHash + couples the wire to the DB schema
func (h *UserHandler) Get(c *gin.Context) {
    var u User
    h.db.First(&u, c.Param("id"))
    c.JSON(http.StatusOK, u)
}

// Good - explicit response DTO; only the fields the contract promises
func (h *UserHandler) Get(c *gin.Context) {
    var u User
    h.db.First(&u, c.Param("id"))
    c.JSON(http.StatusOK, UserResponse{ID: u.ID, Email: u.Email, Name: u.Name})
}
```
...
Use skill: `review-report-writer` with `report_type: review-api` and every required input: `report_body`, `branch` (from the handle), refs from the precondition handle, `base_sha` / `head_sha` from Step 3 (whole-service sweep: both = `HEAD`), `stack: go-gin`, `scope: +api`, `depth` as resolved from the Depth table, and `mode: full`, `round: 1` - unless `review-api-<branch>.md` already exists with valid frontmatter, then increment its `round` and pass its `head_sha` as `prior_head_sha`. (The handle's `prior_checkpoint` is keyed to the general review report - do not use it here.) Write before ending; print confirmation.
...
```markdown
## Go API Review Summary

**Stack Detected:** Go <version> / Gin <version>
**OpenAPI Spec:** swaggo | committed openapi.yaml | none detected
...
1. **Location:** [file:line or route]
   **Issue:** [name the gap: raw GORM entity in `c.JSON`, removed response field on `/v1/`, error leaks `gorm.ErrRecordNotFound`, unpaginated `Find(&all)`, verb in path, etc.]
   ...
   **Fix:** [specific: response struct, `/v2/` + `Sunset`, RFC 9457 envelope, cursor pagination, `Idempotency-Key` header, etc.]
...
- Returning a raw GORM entity / `gorm.Model` from `c.JSON` (over-exposure + DB-schema-to-wire coupling)
- Leaking a Go stack trace, `gorm.ErrRecordNotFound`, or an internal ID in an error response

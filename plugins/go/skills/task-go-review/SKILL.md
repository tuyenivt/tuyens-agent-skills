...
- [ ] **Response-DTO field-stripping audit (when a response DTO IS used)**: open the response DTO struct and compare its `json:` field list against the model's persisted columns. Any of these on the model but missing from the DTO that's currently being included is a finding: `PasswordHash` / `EncryptedPassword`, `MFASecret` / `OTPSecret` / `RecoveryCodes`, `APIKey` / `WebhookSecret`, `InternalNotes` / `AdminNotes` / `AuditLog`, `IsAdmin` / `Role` / `InternalRole`, `TenantInternalFlags`, `DeletedAt` (soft-delete metadata), `InternalCreatedBy` / `LastLoginIP`. Mirror smell: a field on the response DTO that doesn't exist on the model (typo or rename drift). Raw `c.JSON(200, *model.User{...})` with no DTO at all is `[High]` regardless of which fields exist today, because adding `MFASecret` to the model later silently exposes it
...
- [ ] **HTTP `Idempotency-Key` header on retry-prone POSTs (distinct from worker-side dedup)**: any non-idempotent `POST` whose client may retry on a network blip - `POST /payments`, `POST /orders`, `POST /refunds`, `POST /subscriptions`, `POST /webhooks` (when the *upstream* retries on 5xx) - must accept and honor an `Idempotency-Key` request header. Pattern: extract via `c.GetHeader("Idempotency-Key")`, look up in a `request_idempotency` table keyed by `(tenant_id, idempotency_key)` storing `(response_status, response_body, created_at)`; on first hit, run the handler and persist the response inside the same transaction; on subsequent hits with the same key, replay the persisted response. This is a separate concern from `asynq.TaskID(businessKey)` (worker-side dedup against task-queue retries) - one protects the client→server boundary, the other protects the worker side. A `POST /payments` with `asynq.TaskID(orderID)` but no HTTP `Idempotency-Key` will still double-charge if the client retries before the server response arrives, because the client-side retry creates a *second* DB row + enqueue before the dedup table sees the first request finish

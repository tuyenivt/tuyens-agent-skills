...
2. **`npm ci` skip via stamp file keyed on Node version.** Run `npm ci` when `node_modules/.install-stamp` is missing, older than `package-lock.json`, OR its first line does not equal the current `node --version`. After a successful `ci`, write `node --version` to `node_modules/.install-stamp`. This prevents a stale `node_modules` from a different Node version (e.g. via `nvm use`) from being silently reused. First run never has the stamp - that *is* the skip condition firing correctly.
...
7. **Reports under `reports/<runId>[__<matrixKey>-<shardIndex>]/`.** JUnit XML + line log + compose log + traces + videos + screenshots. The suffix appears only when `REGRESSION_MATRIX_KEY` or `SHARD_INDEX` is set; otherwise the path is exactly `reports/<runId>/`. Sharded runs must merge JUnit via `junit-merge` after CI gathers per-shard artifacts.
...
MATRIX_SUFFIX=""
if [ -n "${REGRESSION_MATRIX_KEY:-}" ] || [ -n "${SHARD_INDEX:-}" ]; then
  MATRIX_SUFFIX="__${REGRESSION_MATRIX_KEY:-default}-${SHARD_INDEX:-1}"
fi
REPORTS=".regression/reports/${REGRESSION_RUN_ID}${MATRIX_SUFFIX}"
PW_ARGS="${REGRESSION_PW_ARGS:-}"
mkdir -p "$REPORTS"
export TZ=UTC

teardown() {
  echo "[runner] capturing compose logs"
  docker compose -p "$PROJECT" -f .regression/docker-compose.regression.yml \
    --profile "$REGRESSION_PROFILE" logs --no-color --timestamps \
    > "$REPORTS/compose.log" 2>&1 || true
  echo "[runner] scrubbing PII (opt-in via config.json#scrub.enabled)"
  .regression/scripts/scrub.sh "$REPORTS" || true   # fail-open
...
# 1. deps. Stamp is keyed on node version + lockfile mtime to catch nvm switches.
( cd .regression && \
  NODE_V="$(node --version)" && \
  STAMP="node_modules/.install-stamp" && \
  STAMP_V="$( [ -f "$STAMP" ] && head -n1 "$STAMP" || echo "" )" && \
  if [ ! -f "$STAMP" ] \
...
# 3b. host/container clock skew check (regression-preflight exit code 23)
.regression/scripts/preflight-clock.sh "$PROJECT" || exit $?
...
done < <(find .regression/seeds -type f \( -name '*.sql' -o -name '*.js' \) | LC_ALL=C sort)
...
- Stamp file: `npm ci` is reproducible but not idempotent; the stamp encodes "did we install for this lockfile already, on this Node version" - keyed on Node version because `nvm use` to a different runtime leaves the old `node_modules` in place.
...
- `apply-seed.sh "$PROJECT" "$FILE"` - dispatches per-engine apply command from `regression-seed-strategy`. Dispatch is by file extension first (`.sql` -> SQL engines from `services.yaml#engine`; `.js` -> mongodb), then by engine. A `.js` file with engine `postgres` -> error before exec.
...
.regression/reports/<runId>/
  junit.xml
  line.log
  compose.log         # captured before teardown (all service stdout/stderr with timestamps)

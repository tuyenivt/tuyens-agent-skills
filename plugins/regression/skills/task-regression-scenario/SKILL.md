...
| `--check <token>`              | Append a check to the flow's `checks:` list for this scaffold. Repeatable. Valid tokens: `contract`, `perf`, `a11y`, `security-headers`, `otel-span:<name>`, `log:<key>`, `metric:<name>`. Routes to the corresponding check atomic per `regression-scenario-author` Rule 13.  |
| `--archetype <name>`           | Set or override the flow's `archetype:` for this scaffold. Names come from `regression-flow-archetypes`. Missing required archetype params abort emission with the per-archetype message.                                                                                       |
| `--negative <description>`     | From-story mode only. Provide the negative-path failure mode when the story did not state one. Bypasses the negative-path gate prompt in Step 3.                                                                                                                                |

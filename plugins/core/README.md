...
**Routers and direct workflows.** Most `task-code-*` skills (`review`, `review-perf`, `review-security`, `review-observability`, `test`) are thin routers: they detect the stack and delegate to `task-{stack}-*` (e.g., `task-rails-review-perf`). For best results, install the matching language plugin and call the stack-specific skill directly to avoid the routing layer. `task-onboard` instead **composes** stack-specific atomics into a single workflow output - it remains a full direct workflow even when a stack matches.
...
| `review-change-intent`       | Phase 0 of an umbrella review: a Change Brief (what was requested, what was delivered, which author decisions the request did not imply, what to verify by hand) plus acceptance-criteria traceability when a requirement source resolves. `Met` requires an implementation anchor and a proof anchor; criteria are never derived from the diff. |
| `review-precondition-check`  | Gate code-review workflows: verify clean tree, non-trunk head, locally-resolvable head ref, confirm head vs current branch when they differ, surface prior-round checkpoint (frontmatter of `review-<head>.md`) so the workflow can recognize a re-review and set the round. Local git only. |
...
| `architecture-guardrail`     | `task-onboard`, `task-code-review`                                                                                         |
| `backend-coding-standards`   | `task-onboard`, `task-code-review`                                                                                         |
| `architecture-concurrency`   | `task-code-review`, `task-code-review-perf`                                                                                |
...
| `review-change-intent`       | `task-code-review` and every `task-*-review` stack umbrella, as Phase 0; not used by the lens workflows                    |
...
4. Otherwise review the full `base...head` range again - risk, blast radius, scope signals, and depth are re-scored on the whole PR every round - and reconcile prior High-Impact Findings as Addressed / Still open / Obsolete / Needs re-check. Open items fold into Next Steps with `(open since round <N>)`.

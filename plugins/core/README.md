...
| `task-code-review`               | **Router.** Detects stack and dispatches to `task-{stack}-review`, forwarding scope (`+perf`/`+sec`/`+obs`/`+rel`/`+api`/`full`) and depth flags. For unknown stacks, runs a minimal Phases-A-E generic review.                                                                                                                                                     |
...
| `task-code-review-api`           | **Router.** Detects stack and dispatches to `task-{stack}-review-api`. For unknown stacks, runs a minimal generic review (contract compatibility / REST design / versioning / pagination / error shape / OpenAPI drift).                                                                                                                                            |

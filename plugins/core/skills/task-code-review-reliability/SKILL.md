...
**Round gate (standalone only; on a sweep it runs after the sweep announcement, with `branch` = the trunk name).** Before reviewing, check `review-reliability-<branch>.md` (`<branch>` = head short name, for `head_ref: HEAD` the handle's `current_branch`; writer filename rules; the handle's `prior_checkpoint` is keyed to the general review report - never use it here). If it exists with valid frontmatter, its `head_sha` equals the current head, and the requested depth does not exceed its `depth` (`deep` exceeds `standard`), print `No new commits since prior reliability review.` and stop - no review, no report. Otherwise set `round` = its `round` + 1 and `prior_head_sha` = its `head_sha`; absent file, or frontmatter missing/unparseable (legacy - the Step 5 write overwrites it) -> `round: 1`, no `prior_head_sha`.

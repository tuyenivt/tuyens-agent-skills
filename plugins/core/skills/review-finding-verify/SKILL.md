...
- **De-escalate untouched pre-existing findings once, never below `[Recommend]`.** A `Pre-existing` finding on code the diff neither changed nor made newly reachable drops `[Must]` -> `[Recommend]`. It is context for the author, not a merge blocker for this PR. `[Recommend]` findings keep their label. Severity of the defect does not exempt it - a crash or vulnerability the PR did not introduce and did not make newly reachable still de-escalates, because blocking this PR would not fix it. The label tracks "must fix before merging this," not "how bad is it."

...
This skill has no atomic skill dependencies. It is self-contained.

## Avoid

- Asking for the full original skill output to be re-pasted - that is unnecessary
- Generating long summaries or analysis in the chat - the log file is the output
- Creating separate files per skill - all feedback goes in one `skill-feedback.md`
- Treating low ratings as errors - they are learning signals, not failures
- Adding timestamps beyond the date - date-level granularity is sufficient

## Success Criteria

- [ ] A new entry is appended to `skill-feedback.md` with skill name, date, and rating
- [ ] Partial or Poor ratings include at least one concrete "what was adjusted" note
- [ ] Suggested fixes are specific enough to act on without asking follow-up questions
- [ ] The file remains readable as a chronological log - no entries are modified or removed
- [ ] Pattern summary is offered if 3+ entries exist for the same skill

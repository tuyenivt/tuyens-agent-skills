...
2. If unset, check `main`, `master`, `develop`, `trunk` via `git rev-parse --verify`, probing `refs/heads/<name>` then `refs/remotes/origin/<name>` (a trunk whose local branch was deleted still tracks remotely; probing only local names silently picks the wrong base). Exactly one name resolves: use it; record `base_source: trunk-fallback`. Two or more resolve: ask the user which is the base; record `base_source: user-prompted`.

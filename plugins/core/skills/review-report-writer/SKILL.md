...
- Validate every input before writing anything, and report all failures at once - one line per bad field, in the input table's row order, so the caller fixes them in a single pass instead of rediscovering them one re-run at a time:

...
Use skill: `stack-detect` to identify the project's language, framework, build tool, and ecosystem. If it finds nothing, use the stack stated in the request. Where the upgrade targets one module of a polyglot repo, detect that module's stack rather than the repo's primary one, and name the module on the Summary's `Stack:` line.
...
Per change relevant to the codebase: change, migration action, effort (S/M/L/XL). A version-forced API relocation (a package move) is `rename`; `transitive` is a pulled-in dependency whose new version changes behaviour in your code. A pin conflict with no code change, like a runtime or build-tool minimum with none, is a Step 3 compatibility row, not a breaking change.

Use skill: `ops-backward-compatibility` for API and data contract impact.

### Step 3 - Compatibility Check

Surface conflicts: transitive dependency version mismatches, build-tool minimums (Gradle/Maven/Bundler/npm), runtime minimums (JDK, Node, Python, Ruby), test library compatibility, and an unpinned dependency the deploy relies on (the app server) - a row here even when absent from the manifest. Mark each Resolvable, Needs own upgrade, Needs own upgrade [VERIFY], or Blocker (the Output's Severity enum). `Blocker` here means no compatible release exists at all; `Needs own upgrade` means one exists and must ship first; neither is the lens severity of the same name that Review Mode uses. Offline, a third-party plugin whose compatible release cannot be confirmed is `Needs own upgrade [VERIFY]`, not Blocker.
...
**Multi-dependency upgrades.** A request is multi-dependency when it names more than one dependency, or when the manifest pins transitive versions the target release moves - a framework bump that forces new pins for its migration tool, API docs or test containers is multi-dependency even though one dependency was named. When bumping several deps at once (framework + runtime + type system), check pairwise compatibility - individually safe upgrades can conflict when combined. Sequence across separate PRs unless the batched changes are inseparable - one cannot build or run without the other - so that each deploy carries one risky change whose rollback is attributable (`ops-release-safety`'s rule); a framework bump batched with a runtime bump never qualifies. Inseparable pins ride in the PR of the dependency that forces them.

### Step 4 - Security Assessment

State the security status of both versions regardless of whether the upgrade is security-motivated: the current version's EOL status, CVEs open on it, which of them the target fixes, and the risk of staying. A target in `Maintenance` is a Go with the next-target date as a Next Step; `Unknown [VERIFY]` on the target puts verifying its support in the verification slot that heads Next Steps. Check the target's own support status too - a target already past EOL is `No-Go - Block` unless a later supported version is out of scope for a stated reason - a named constraint such as a larger migration deferred to a later quarter, not merely the requester having named this version. If listing CVEs, cite source rather than speculating; offline, name known CVEs tagged [VERIFY] - never invent IDs.

### Step 5 - Migration Effort Estimate

Aggregate the effort from Step 2 and any compatibility work from Step 3. Total = the largest component effort, bumped one size when two or more components share that size **and that size is M or above**; `None` components are skipped, not counted toward the bump - three S components stay S, since a small config change does not become medium by having three parts - and XL never bumps further. The Size guidance below profiles the upgrade as a whole and sets a floor for the Total only when the change class it names is actually present - a runtime *major* upgrade is XL; a minor or feature release of the same runtime takes the size its inventory earns; a patch bump is not. It does not classify individual components. Per-dependency verdicts use each dependency's own effort with the batch Risk Level. For multi-dependency requests, render one breakdown table per dependency, then a batch total aggregated from the per-dependency totals by the same rule (XL does not bump further):

| Component                | Effort   | Notes                                          |
| ------------------------ | -------- | ---------------------------------------------- |
| Breaking change fixes    | None/S/M/L/XL | Per change or aggregate                   |
| Compatibility resolution | None/S/M/L/XL | Transitive conflicts, build tool          |
...
- **XL**: Architectural changes, namespace migrations, runtime major upgrade - should be a dedicated epic
...
| ------------------- | ----------------- | ----------------------------------------- |
| Breaking changes    | None/Low/High     | High = removals, behavior changes, or a runtime major in the path (whether or not it forced code changes); Low = renames, config, minor runtime bumps, or transitive bumps that resolve cleanly; None = none in the inventory and no runtime major |
...
Define before the upgrade starts: trigger (specific signal - error rate, latency, failed smoke), procedure, data compatibility (schema/config/message-format changes complicate rollback), time window, exposure control (`ops-release-safety`'s rollout strategy - feature flag, canary, blue-green, rolling update - or `one-shot script` for a run-once data or config step, or `big bang (as proposed) - {why the topology allows nothing finer}` - never selected on a Wide blast radius, where the missing finer control becomes a prerequisite Next Step and the verdict is at most Go - Planned), deploy order (from `dependency-impact-analysis` when separately deployed components upgrade together), and the point of no return past which the plan rolls forward instead. A rate trigger carries `ops-release-safety`'s sample requirement: enough requests in the window to tell the trigger from the baseline (thousands for a sub-percent gap), or an absolute count where the slice cannot reach it.
...
Produce a clear recommendation, then the Recommended Next Steps in this order: the verification slot (Step 2's changelog verification, Step 4's target-support verification - offline only), the first action the recommendation implies, then every prerequisite Steps 3, 4, 6 and 7 raised (a `Needs own upgrade` row's own PR first among them); and the Assumptions list (every default taken for missing context). Apply these tests in order and stop at the first that fires, so every value stays reachable:

1. An unresolved compatibility `Blocker`, or a target version that is itself unsupported with no Step 4 reason a later supported one is out of scope -> **No-Go - Block**.
...
3. Effort XL, or cross-team coordination or an infrastructure change (a runtime or build-tool *major*, new OS libraries - not a minor or patch bump) is required -> **Go - Epic**.
4. Effort L, or risk Medium/High -> **Go - Planned**.
5. Effort S/M and risk Low, one PR -> **Go - Now**. Any multi-PR shape - a `Needs own upgrade` row shipping first, or a batch sequenced by Step 3 - is **Go - Planned** whatever the sizes, since the shape needs scheduling.

The ladder is total: every assessment lands on exactly one rung.

The table restates the same tests:

| Recommendation | When                                                                             |
| -------------- | -------------------------------------------------------------------------------- |
| Go - Now       | Effort S/M, risk Low, one PR                                            |
| Go - Planned   | Effort L, risk Medium/High, or any multi-PR shape - schedule as a dedicated sprint item with testing |
...
| No-Go - Block  | An unresolved `Blocker` in the Compatibility table, or a target version that is itself unsupported with no stated out-of-scope reason |

State the primary reason, weighing stated constraints (deadlines, freeze windows, team rotation) - name them in it. A stated deadline or window for this change counts as "asked" for calendar time: map the PR sequence to the window on the Summary's `Window fit` line; a third party's own notice period is a constraint on one code path, not a window. For multi-dependency requests, give a per-dependency verdict plus a batch verdict. Sequencing is a shape, not a verdict: when the deps are individually fine but unsafe batched, the Summary Recommendation stays a single enum value (`Go - Planned`, or `Go - Epic` at XL - rung 5 defers to this), the Primary Reason says the batch was rejected in favour of sequenced PRs, and the Sequence column carries the order. `No-Go - Block` is reserved for deps that cannot ship at all. The Risk Assessment table covers the batch; the Rollback Plan follows the shipped shape - one plan for a batched upgrade, or one row per PR carrying every field the bullet form carries when sequenced.
...
| Risk assessment              | Yes      | All four factors levelled (breaking changes, test coverage, rollback complexity, blast radius) and a summary level derived |
| Compatibility conflicts      | Yes      | Transitive, build tool, runtime conflicts surfaced, each marked Resolvable / Needs own upgrade{ [VERIFY]} / Blocker |
| Multi-dependency interaction | Yes*     | When upgrading multiple deps, interaction effects and required ordering assessed  |
| Security status              | Yes      | Current version EOL/CVE status stated, even when upgrade is not security-driven   |
| Effort estimate              | Yes      | S/M/L/XL per component (breaking changes, compatibility, test and validation)     |
| Rollback plan                | Yes      | Trigger, procedure, data compatibility, time window, exposure control, deploy order - not just "revert version" |
...
- **Upgrade assumed safe without changelog review** (any version distance): Blocker for ecosystems known to break in minor versions (Spring Boot, Django, Rails); Major otherwise; Minor on a patch bump
...
- [ ] behavioral-principles loaded before any step; Step 1's stack detected and carried on the review's context line, scoped to the module under review in a polyglot repo
...
- **Stack:** {detected language / framework}{, module <name>}
...
- **Window fit:** {the PR sequence mapped to the stated deadline or freeze}  <!-- only when a deadline or window was stated -->

## Per-Dependency Verdicts  <!-- multi-dependency requests, and any single-dependency request whose Needs-own-upgrade row ships first: one row per PR -->
...
| --------------------------------- | --------------- | -------------------------------- | ---------------- | ---------- |
| {API or behavior removed/changed}{ [VERIFY]} | {e.g., 3.4}     | {removal \| rename \| behavior \| config \| transitive \| runtime} | {what to do}     | {S/M/L/XL} |

## Compatibility Issues

| Conflict              | Severity                                  | Resolution       |
| --------------------- | ----------------------------------------- | ---------------- |
| {dependency conflict} | {Blocker / Needs own upgrade / Needs own upgrade [VERIFY] / Resolvable} | {how to resolve} |
...
<!-- multi-dep: one `### {Dependency}` subheading carrying this table per dependency, then a final `### Batch total` line: one size aggregated from the per-dependency totals by the Step 5 rule -->
...
| **Total**                | **{S/M/L/XL}** | {largest component, bumped by the Step 5 rule; never None since Test and validation is always sized} |
...
<!-- sequenced multi-dep: replace the bullets with one table: | PR | Trigger | Procedure | Data compatibility | Exposure control | Deploy order | Point of no return | Window | -->
...
- **Exposure control**: {feature flag | canary | blue-green | rolling update | one-shot script | big bang (as proposed) - {why nothing finer exists}}
- **Deploy order**: {component order from dependency-impact-analysis | single deployable}
- **Point of no return**: {the step after which the plan rolls forward, or none}

## Recommended Next Steps

1. {verification items when offline, else the first action based on recommendation - e.g., "Create spike ticket to validate migration path"}
2. {Second action}
3. {Third action}
...
- [ ] Step 3: compatibility conflicts surfaced (transitive, build tool, runtime), each marked Resolvable / Needs own upgrade{ [VERIFY]} / Blocker; multi-dependency requests checked pairwise
...
- [ ] Step 8: Go/No-Go recommendation is a single enum value, actionable for a spike ticket; per-dependency verdicts and PR sequence given when multi-dependency; Recommended Next Steps and Assumptions filled

...
| ---------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------- |
| architecture-architect | Design authority: system design, reviewer-facing design briefs, and migration planning (decomposition, consolidation, modernization, schema change) - authoring and review. | `task-design-architecture`, `task-design-brief`, `task-migrate-architecture` |
...
| oncall-responder       | Incident responder / SRE walking the live incident lifecycle: alert triage and routing, containment-first root-cause analysis.                              | `task-oncall-triage`                                    |
...
| `task-design-brief`                 | Design an epic as a two-page, diagram-first brief written to win approval: problem, approach, change inventory, reviewer-framed risk, rollout and back-out, and the decisions being asked for. Calibrates to the reviewer's architecture and domain fluency and follows a company template when one exists. Review Mode returns a soundness verdict plus a separate approval-readiness verdict. |
...
| `task-oncall-triage`                | Triage an incoming alert, ticket, or symptom in two phases: a fast provisional First Look (work type, severity, immediate action), then a deep-pass subagent that runs the routed workflow and confirms or overturns it.                                                             |

The design pipeline: `task-design-architecture` produces the design and `task-breakdown-design` turns it into a task graph (or, in review mode, critiques a graph someone else wrote). `task-design-brief` is an alternative to `task-design-architecture`, not a step after it - same analysis, a two-page reviewer-facing document instead of a twelve-section record. Pick by who consumes the artifact: an architecture board, compliance file, or cross-team contract needs the record; an approver who has to sign off needs the brief. Either one feeds `task-breakdown-design`. `task-migrate-architecture` covers the other direction - moving an existing system to a new state rather than designing a new one. All target the architect or tech lead who owns the build. `task-oncall-triage` covers live incident work - triage, route, and drive to a confirmed root cause.
...
| `backend-caching`               | Caching strategy, invalidation, stampede protection, key design, TTL sizing, and response payload shaping. Adapts to the detected stack. | `task-design-architecture`                                                                   |
| `architecture-review-lens`      | Review lens for architecture artifacts - severity taxonomy, completeness audit, consistency check, criteria scoring, verdict | All 5 authoring workflows in their Review Mode                                                |
| `design-audience-calibration`   | Calibrate a design doc to its reviewer - architecture and domain fluency axes set vocabulary, glossing, diagram density, and appendix policy | `task-design-brief`, `task-design-architecture`                                               |
| `design-reference-pattern`      | Extract a design-doc house pattern from a company template or approved prior design - section skeleton, depth, diagram and metadata conventions | `task-design-brief`, `task-design-architecture`, `task-migrate-architecture`                  |
...
| `oncall-investigate`            | Structured investigation for non-incident oncall work - support tickets, operational questions, unexpected behavior, performance concerns | `task-oncall-triage`                                                             |
| `incident-root-cause`           | Active production incident investigation with blast radius assessment and containment-first analysis                         | `task-oncall-triage`                                                                         |
...
| `ops-observability-fetch`       | Fetch evidence (issues, metrics, logs, traces, deploys, monitors) from Sentry/Datadog/Honeycomb MCP; paste-mode fallback     | `task-oncall-triage`, `oncall-investigate`, `incident-root-cause`, `log-analysis`            |
...
| ----------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `task-design-architecture`          | `nfr-specification`, `architecture-review-lens` (review mode), `system-boundary-design`, `tradeoff-analysis`, `stack-detect`, `architecture-guardrail`, `review-blast-radius`, `architecture-data-consistency`, `backend-idempotency`, `backend-caching`, `ops-resiliency`, `ops-failure-classification`, `failure-propagation-analysis`, `ops-observability`, `backend-db-indexing`, `architecture-capacity`, `ops-release-safety`, `dependency-impact-analysis`, `architecture-concurrency`, `ops-engineering-governance`, `backend-api-guidelines`, `ops-backward-compatibility`, `design-audience-calibration` and `design-reference-pattern` (when a reviewer profile or reference doc is supplied) |
| `task-design-brief`                 | `stack-detect`, `design-audience-calibration`, `design-reference-pattern`, `review-blast-radius`, `tradeoff-analysis`, `ops-release-safety` (always); `backend-db-migration`, `ops-backward-compatibility`, `system-boundary-design`, `architecture-data-consistency`, `backend-idempotency`, `ops-resiliency`, `failure-propagation-analysis`, `dependency-impact-analysis`, `ops-feature-flags`, `nfr-specification`, `ops-observability` (signal-triggered); `architecture-review-lens` (review mode) |
| `task-migrate-architecture`         | `stack-detect`, `design-reference-pattern` (when a reference doc is supplied), `system-boundary-design`, `strangler-fig-pattern`, `architecture-guardrail`, `architecture-data-consistency`, `tradeoff-analysis`, `review-change-risk`, `backend-db-migration`, `backend-db-indexing`, `backend-idempotency`, `ops-backward-compatibility`, `review-blast-radius`, `dependency-impact-analysis`, `failure-propagation-analysis`, `ops-resiliency`, `ops-observability`, `ops-release-safety`, `ops-feature-flags`, `architecture-review-lens` (review mode) |
...
| `task-oncall-triage`                | `oncall-investigate`, `incident-root-cause`, `ops-observability-fetch`, `stack-detect`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
**Get an epic approved (two-page brief, calibrated to the reviewer):**

```
/task-design-brief
Epic: Send order confirmations by SMS as well as email
Current system: Rails monolith, orders table, nightly settlement job reads orders.status
Reviewer: promoted-from-dev team lead, 9 years on this system, no architecture background
Reference: docs/design-template.md
```

The reviewer profile is optional and takes two axes, each `High` or `Low` - architecture fluency and domain fluency. Unstated, it assumes Architecture: Low, Domain: High (the long-tenure approver) and says so on the document's first line. The reference doc is optional too; supply a company template, an already-approved design, or neither.

Instead of passing `Reference:` every time, put the pointers in the consuming project's `CLAUDE.md`:

```markdown
## Design Docs

Template: docs/design-template.md
Reference: docs/designs/2026-04-payments-split.md
...
/task-design-brief
[paste a brief - a colleague's, or your own draft before you send it]
```

`task-design-brief` adds a second verdict on top of the shared one: an approval-readiness call (`Ready to send` / `Send with fixes` / `Not ready`) listing what the reviewer will trip on, phrased as the objections they would raise. Run it on your own draft to find them before the reviewer does.
...
**Triage an alert, ticket, or symptom:**

```
/task-oncall-triage
[paste the alert, ticket, Slack message, stack trace, or Sentry/Datadog URL to triage]
```

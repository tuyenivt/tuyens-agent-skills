...
| Severity     | Definition                                                                                                                                                                                                                                                             |
| ------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Critical** | Unauth RCE or auth bypass; working SQLi on prod path; `eval`/`shell_exec`/`system($userInput)`; secrets / `APP_KEY` committed; `unserialize` on untrusted input; `{!! $userInput !!}` on privileged surface; `$guarded = []` on User/Role with `Model::create($request->all())` exposing role. Blocks merge. |
...
**Combined-finding rule.** When findings *compose* on the same code path into a worse threat than either alone, file as a single finding at the elevated severity and cite each component.

Canonical compositions:
- Missing `auth:` middleware + `User::find($id)->update($r->all())` + missing Form Request/Policy = **Critical** unauth admin takeover
- Missing `$this->authorize` + `Model::create($request->all())` with `$guarded = []` = **Critical** authenticated admin override
- SSRF via `Http::get($userUrl)` reachable from unauthenticated endpoint = **Critical** unauth SSRF
- `unserialize($webhookBody)` + signature compared via `==` = **Critical** RCE-via-forged-signature
...
| Invocation                               | Meaning                                                                                                |
| ---------------------------------------- | ------------------------------------------------------------------------------------------------------ |
| `/task-laravel-review-security`          | Review current branch vs its base - fails fast if on a trunk branch                                    |
...
When invoked as a subagent of `task-code-review-security` (parent passes precondition-check handle plus already-read diff and commit log), Step 3 is skipped.
...
Use skill: `stack-detect` to confirm PHP / Laravel. Skip when parent already detected. If not Laravel, stop. Detect auth (Sanctum token / Sanctum SPA / Passport / session) and ORM. Record `Auth`, `ORM`, `Tests Framework`.

### Step 3 - Resolve the Diff Under Review

Use skill: `review-precondition-check`. On approval, read `git diff <base>...<head>` and `git log <base>..<head>` once. Skip with parent-supplied artifacts. Never run state-changing git.
...
- Every changed Policy, controller action, Form Request, model (`$fillable` / `$guarded` / `$hidden`), query (parameterization), migration (PII, FK on tenant scoping, privilege widening)
...
When the diff removes middleware or relaxes auth, `git log -p` the prior revision.

### Step 5 - OWASP Triage (Laravel Lens)

A **triage pass**, not a separate findings list. Produce one signal verdict per category that fires; omit categories with no signal in the diff to keep the table actionable. Steps 6-7 produce findings.

| Risk                          | Laravel-specific check                                                                                                                                                                                                                       |
| ----------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Broken Access Control         | `$this->authorize` / `Gate::authorize` / `can:` middleware / ownership scoping. Bare `Order::find($id)` on user-owned data is IDOR. Form Request `authorize()` returns explicit boolean.                                                     |
| Injection                     | Eloquent / `DB::select(..., [bindings])` / `whereRaw('col = ?', [$input])` parameterized; `whereRaw("col = '$input'")` and `orderByRaw($request->input('sort'))` without allowlist are injection. `eval`/`exec`/`shell_exec` with user input is RCE. |
| Cryptographic Failures        | `Hash::make` only; `APP_KEY` from env; `Crypt::encryptString` for two-way; `random_bytes` / `Str::random` not `mt_rand`.                                                                                                                     |
...
| Vulnerable Components (A06)   | `composer audit` clean; `roave/security-advisories` in `require-dev`; `composer.lock` committed.                                                                                                                                             |
| Data Integrity Failures (A08) | `unserialize($userInput)` is RCE-via-gadget-chain. File uploads bounded by size and validated MIME (`mimes:` + magic-byte). Mass assignment flagged.                                                                                          |
| Logging & Monitoring (A09)    | `Log::*` / `Monolog` never log `password`, `token`, `Authorization`, `Cookie`. Auth events logged. Sentry/Bugsnag `beforeSend` strips PII.                                                                                                    |

### Step 6 - Authentication, Authorization, Input Validation, and Common Vulnerability Patterns

Use skill: `laravel-security-patterns` for canonical implementation. Sub-scans are review-scoped checks on the diff; each check appears in exactly one sub-scan.

**Authentication scan:**

- [ ] Sanctum `stateful` domains explicit (not `*`); token `expiration` finite; `EnsureFrontendRequestsAreStateful` only on SPA routes
- [ ] `APP_KEY` from env; `Hash::make` / `Hash::check` only; `Hash::needsRehash` on long-lived auth
- [ ] `/login`, `/password/reset`, `/password/email` rate-limited (`throttle:auth` 5/min by IP); `MustVerifyEmail` + `verified` middleware where required
- [ ] Session cookies `secure`, `http_only`, `same_site='lax'` in prod; `auth:` middleware before authorization
...
- [ ] Lookups scope through principal (`$user->orders()->findOrFail($id)`); multi-tenant queries scoped at global-scope / query layer
- [ ] CORS `allowed_origins` an explicit list; CSRF middleware on cookie-session / Sanctum-SPA routes; `validateCsrfTokens(except: [...])` limited to webhooks

**Input validation + mass assignment scan:**

- [ ] Every input-accepting action uses a typed Form Request; `rules()` covers every field with specific constraints (`email:rfc,dns`, `url:http,https`, `numeric|min:|max:`, `Rule::in([...])`)
- [ ] `Model::create($request->validated())` not `$request->all()`; no `$guarded = []`; explicit `$fillable`; server-set fields assigned outside fillable
- [ ] No privilege / identity fields (`role`, `is_admin`, `id`, `tenant_id`) in user-facing Form Request rules; `$hidden` on User-like models; no raw Eloquent model returned (always API Resources)
- [ ] **File uploads**: validated via magic bytes (`finfo`), not `getClientMimeType`; per-file size limit; non-public disk; generated UUID filename; canonicalized save path
...
- [ ] **SQL injection**: `whereRaw` / `DB::raw` / `orderByRaw` / `DB::select` use bindings; user-supplied `sort` validated against `Rule::in([...])`
- [ ] **Deserialization**: `unserialize($userInput)` is Critical regardless of context; audit `Crypt::decrypt` for downstream `unserialize`
- [ ] **XSS / SSTI**: `{!! $userInput !!}`, `Response::make($html)` from concatenation, `Blade::compileString` / `Blade::render` on user input
- [ ] **Variable injection / LFI**: `extract($userArray)`, `include $userControlled`, `require $userControlled`
- [ ] **Open redirect**: `redirect($userInput)` validated via host allowlist; reject `//evil.com`, `data:`, `javascript:`; prefer `redirect()->intended()`
- [ ] **CSPRNG**: `random_bytes` / `Str::random` / `Str::uuid`; never `rand` / `mt_rand` / `uniqid`. **Timing attacks**: `hash_equals` for HMAC / signature
- [ ] **Debug exposure**: `APP_DEBUG=true` in non-local is Critical; Telescope / Horizon / Pulse gated by Gate
- [ ] **`env()` outside config files** flagged; **`.env.example`** carries placeholders only; real-looking secrets in history require rotation + `APP_KEY` rotation
- [ ] **SSRF**: user-controlled URLs into `Http::*` / `Storage::download` / `file_get_contents` validated against allowlist rejecting cloud metadata, localhost, RFC1918, link-local; re-resolve at request time (DNS rebinding)
...
- [ ] **Dependency hygiene**: `composer audit` clean; `roave/security-advisories` in `require-dev`

### Step 7 - Data Protection

_Skip when no PII signals in the diff (no new columns containing user data, no encryption/decryption changes, no logging of model objects)._

- [ ] **PII encrypted at rest** via `'encrypted'` cast (AES-256-CBC with `APP_KEY`) or `Crypt::encryptString`; MySQL `AES_ENCRYPT` is not authenticated
- [ ] **`$hidden`** populated (`password`, `remember_token`, `two_factor_secret`, `api_token`); **log redaction** via Monolog processor
- [ ] **No sensitive data in URLs**; **TLS** via `URL::forceScheme('https')` + `TrustProxies`; backups encrypted
- [ ] **Secrets management**: env from secret store (Vault / AWS Secrets Manager); `.env` sourced at deploy time; `php artisan env:encrypt` for committed encrypted env files
...
- [ ] Step 3: `review-precondition-check` ran; diff and commit log read once
- [ ] Step 4: Security surface read directly; prior revision consulted when middleware removed
- [ ] Step 5: OWASP triage produced a verdict for each fired category; non-firing categories omitted
...
- [ ] Step 7: Data protection assessed when PII signals present; otherwise skipped explicitly
...
[Only categories with a signal in the diff; one row per fired category.]
| Category | Verdict |
...
- Findings without an attack scenario

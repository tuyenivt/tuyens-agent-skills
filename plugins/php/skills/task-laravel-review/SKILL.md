...
- [ ] **Idempotency on state-mutating writes**: `POST /orders`, `POST /payments`, `POST /charges` and similar create-or-charge endpoints accept an `Idempotency-Key` header from the client; the server stores `(client_id, idempotency_key) -> response_hash` in a dedupe table (or Redis with TTL) and returns the cached response on retry. Without this, a network retry or double-click produces duplicate orders / charges. This is endpoint-level idempotency, distinct from the queue-job idempotency check above (job idempotency protects against at-least-once delivery; HTTP idempotency protects against client retries). Flag missing dedupe on any new `POST` that creates or charges
- [ ] **Response-shape field stripping**: when `from_attributes` / direct-model-return paths exist, compare the API Resource's `toArray()` field list against the model's column list. Any column present on the model but absent from the Resource should be intentional - flag `internal_notes`, `password_hash`, `mfa_secret`, `audit_log`, `tenant_internal_*`, `is_admin` if they exist on the model AND the controller returns the model directly OR uses `JsonResource::make($model)` without `toArray()` filtering

...
**Not for:** Single-task execution (use `task-spec-implement` or stack-specific `task-*-new` directly), features without a spec (use `task-implement`), code review of an existing PR (use `task-code-review`), debugging a broken build (use `task-code-debug`).

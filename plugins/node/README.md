...
| `task-node-review`               | node-tech-lead            | Node staff-level code review umbrella - correctness, architecture, AI-quality, maintainability with NestJS / Express idioms; spawns parallel perf / security / obs / reliability subagents |
...
| `task-node-test`                 | node-test-engineer        | Jest / Supertest strategy / scaffolding (NestJS TestingModule, Testcontainers, MSW, BullMQ handler lane + real-broker lane)                 |
...
| `node-security-patterns` | JWT signing/verify, mass-assignment DTOs, prototype pollution, SSRF resolved-address guard, file upload, webhook signatures, secrets via typed `ConfigService`, `eval`/`vm` prohibitions, open redirect, `child_process.execFile`. Canonical patterns - `task-node-review-security` Step 9 delegates here |

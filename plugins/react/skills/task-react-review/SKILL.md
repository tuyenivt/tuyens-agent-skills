...
For each selected scope, spawn one independent subagent **in parallel** with the main thread. Use the **declared subagent for that scope** (`subagent_type` below) - do not infer the agent from the scope name; an observability review is not a `react-tech-lead` spawn:
...
- **Strongest intent wins** when labels differ across subagent reports for the same finding: `Must` > `Recommend`. Map subagent scales: `Critical` -> `Must`, `High` -> `Recommend`, `Medium` / `Low` -> drop from the merged list (only `Must` and `Recommend` are emitted)
...
- **Preserve deep-only sections** returned by subagents (e.g., reliability's `Failure-Mode and User-Impact Map`) as their own section after Next Steps - they are not findings; the merge must not drop them
...
- `scope` (resolved in Step 4, mapped to the writer's enum: `Core` -> `core-only`, `+Sec` -> `+sec`, `+Perf` -> `+perf`, `+Obs` -> `+obs`, `+Rel` -> `+rel`, `Full` -> `full` - the writer rejects unmapped display values), `depth` (resolved/auto-promoted), `stack = react`

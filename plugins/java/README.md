...
Claude Code plugin for Java 21+ / Spring Boot 4.0+ development.

## Stack

- Java 21+ (examples target Java 25)
- Spring Boot 4.0+ (written against 4.1; Boot 3.x projects get output for the version they run)
- Gradle

## Key Features

- **Virtual Threads**: All skills enforce Virtual Thread compatibility (no `synchronized` around blocking IO on JDK 21-23)
- **Java 21+ Patterns**: Records for DTOs, pattern matching, sealed classes
- **Spring Boot 4.0+**: Spring Framework 7, Jakarta EE 11, Jackson 3, modular starters, optimized connection pools (10-40)
...
- **Spring Security 7.x**: SecurityFilterChain, OAuth2/JWT, method security
- **Gradle**: Version catalogs, convention plugins, build cache optimization
- **Testing**: Spring test slices, Testcontainers 2, JUnit 6, Mockito
...
| `task-spring-test`                 | `java-test-engineer`        | Spring-specific test strategy and scaffolding: JUnit 6, Spring slices (`@WebMvcTest`, `@DataJpaTest`, `@JsonTest`), Testcontainers (PostgreSQL via `@ServiceConnection`), Mockito strict stubbing, Spring Security Test.                                                                                                                                                                                                       |
| `task-spring-review`               | `java-tech-lead`            | Spring-specific staff-level code review umbrella: Phases A-E (risk, correctness, architecture, AI quality, maintainability) with Spring idioms (layer boundaries, fat controllers, JPA-in-API, `@Transactional` self-invocation, anemic domain, `@Autowired` field injection, Virtual Thread pinning). Spawns Spring perf/security/observability/reliability subagents for extra scopes. Delegate of `task-code-review` when stack is Spring Boot. Runs standalone with full PR/branch resolution. |
| `task-spring-review-observability` | `java-observability-engineer` | Spring-specific observability review: Logback + Logstash JSON, MDC correlation, Spring Boot Actuator exposure, Micrometer metrics with bounded tag cardinality, Micrometer Tracing / OTel, Kafka / RabbitMQ listener observation, error-tracker Boot starters. Delegate of `task-code-review-observability` when stack is Spring Boot.                                                                                                                                       |
...
| `spring-security-patterns`       | Spring Security 7.x configuration                                       |
...
| `java-test-engineer`        | JUnit 6, Testcontainers, Spring test slices                                                 |
| `java-security-engineer`    | Spring Security 7.x, OWASP for Java                                                         |

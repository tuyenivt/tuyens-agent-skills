...
- [ ] **Multi-tenant isolation**: tenant scoping enforced at the repository / `@Filter` layer (Hibernate `@TenantId` in 6.x or `@Filter`), not at the controller layer alone. Derived queries that take the tenant/owner as a parameter (`findByIdAndUserId`, `findByIdAndTenantId`) are an acceptable per-call pattern but only when **every** repository read on that aggregate uses one - a single missing variant exposes other tenants' data

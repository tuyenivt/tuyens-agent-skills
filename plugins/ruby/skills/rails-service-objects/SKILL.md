...
`FOR UPDATE` prevents oversell; the `ORDER BY id` in the same locking query (chain order is irrelevant - the SQL is identical) kills the A->B/B->A deadlock two carts sharing products would otherwise hit. For lock semantics, use skill: `rails-db-locking-patterns`.
...
- Post-commit dispatch via `ActiveRecord.after_all_transactions_commit` (7.2+) or `after_commit_everywhere` when nested.
- An inner service signals failure to a composing caller by raising inside the transaction (the caller's rescue sits outside the block, so everything rolls back); a `Result` is read after the block.
...
def create
  ...
  result = Checkout.new(cart: @cart, payment_method_id: params.require(:payment_method_id),
                        idempotency_key: request.headers["Idempotency-Key"].presence || "checkout-#{current_user.id}-#{@cart.id}").call   # derive when the client sends none
  if result.success?
...
Transaction: {Yes - models mutated | Yes - two: claim, then finalize | Yes - key row first, result row after the call | joins the caller's (requires_new: true | fused - GAP if the caller rescues) | No}

External API: {none | provider; placement: before the transaction (gating) | after commit via job (deferrable) | inside - GAP; compensating action: <job name | pending-claim release + sweep | absent - GAP | N/A> - one entry per provider}
...
A field the current code cannot satisfy carries ` - GAP` (a job whose `perform` receives no code; a service that returns nothing).
...
- A `rescue` attached to the `transaction do ... end` block itself - it runs inside the transaction, so a declined charge still commits (`rails-transaction-patterns`)

...
- Live production incident (active outage, error spike, or queue meltdown harming users now): escalate to the team's on-call / incident-response owner. Root-cause triage and the code fix return to this agent once the incident is closed.
...
- Rails code review / refactor: `/task-rails-review` (umbrella with parallel perf / security / observability / reliability subagents). Standalone test strategy: `/task-rails-test`; specs for a feature designed or built here ship inside `/task-rails-implement`. Single-scope depth: the sibling `rails-security-engineer`, `rails-performance-engineer`, `rails-observability-engineer`, or `rails-reliability-engineer`.

...
> This agent drives the Rails-specific performance review workflow `/task-rails-review-perf`. For stack-agnostic performance review, use the core plugin's `/task-code-review-perf`. Behavior under failure or saturation (fallbacks when a dependency is down, load shedding, breaker / retry design) belongs to `rails-reliability-engineer` - a bare slowness report stays here. Instrumentation / tracing strategy (what to instrument, where request time goes) belongs to `rails-observability-engineer`; this agent profiles to diagnose a specific regression. A live production incident escalates to the team's on-call / incident-response owner; the post-incident query / root-cause review returns here once stable. Implementing accepted fixes routes to `rails-engineer` (re-profile and verify here). Bundled slices dispatch to their owners at split time.

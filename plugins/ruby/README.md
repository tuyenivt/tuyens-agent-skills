...
| Agent                        | Description                                                                                                                                                                      |
| ---------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `rails-engineer`            | Rails 7.2+ engineer - builds features end-to-end: migrations, models, services, controllers, serializers, Sidekiq jobs, RSpec. Debugs errors, logs, and failing specs.                                                |
...
| `rails-observability-engineer` | Structured logging (lograge/semantic_logger), ActiveSupport::Notifications, Sidekiq tracing, correlation IDs, query_log_tags, Sentry/OTel wiring.                              |
| `rails-reliability-engineer` | Faraday/Net::HTTP timeouts, Sidekiq idempotency/retries/dead-set, Stoplight circuit breakers, after_commit dispatch, AR pool bounds, graceful degradation and recovery under failure. |
| `rails-api-engineer`         | API-contract review - REST resource design, breaking-change/versioning detection, serializers over AR models, RFC 9457 errors, pagination, rswag/OpenAPI drift. |
...
| --------------------------------- | ---------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `task-rails-implement`            | `rails-engineer`            | End-to-end Rails feature implementation. Generates migrations, models, services, controllers, serializers, Sidekiq jobs, and comprehensive RSpec tests.                                                                                                                                                                                                                                                  |
| `task-rails-review`               | `rails-tech-lead`            | Rails-specific staff-level code review umbrella: 10 steps covering risk, correctness, architecture, code hygiene, and maintainability, with Rails idioms (Zeitwerk, callback abuse, fat controllers, AR-in-API, service-object boundaries). Spawns Rails perf/security/observability/reliability/api subagents for extra scopes. Delegate of `task-code-review` when stack is Rails. Runs standalone with full PR/branch resolution. |
...
| `task-rails-review-observability` | `rails-observability-engineer` | Rails-specific observability review: `ActiveSupport::Notifications`, `query_log_tags`, `lograge`/`semantic_logger`, Sidekiq middleware tracing, Rack correlation IDs, error-tracker gem wiring. Delegate of `task-code-review-observability` when stack is Rails.                                                                                                                                        |
| `task-rails-review-reliability`   | `rails-reliability-engineer`   | Rails-specific reliability review: Faraday/`Net::HTTP` timeouts, Sidekiq idempotency/retries/dead-set, `Stoplight` circuit breakers, `after_commit`/outbox dispatch, AR connection-pool bounds, graceful degradation and recovery under partial failure. Delegate of `task-code-review-reliability` when stack is Rails.                                                                               |
| `task-rails-review-api`           | `rails-api-engineer`         | Rails-specific API-contract review: REST resource design, breaking-change/versioning detection, serializers over raw AR models, RFC 9457 errors, pagination, rswag/OpenAPI drift. Delegate of `task-code-review-api` when stack is Rails.                                                                                                                                                            |

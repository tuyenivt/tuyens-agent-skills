...
| task-python-review                | python-tech-lead            | Python staff-level code review umbrella - Phases A-E with FastAPI/Django idioms; spawns parallel perf/security/obs/reliability subagents |
...
| python-security-patterns   | JWT/OAuth2 (python-jose/pyjwt), Pydantic mass-assignment (`extra="forbid"`) / DRF `read_only_fields`, SSRF allowlist, file-upload magic bytes, pickle/yaml/eval prohibitions, secrets via pydantic-settings |
...
| Agent                       | Description                                                                                            |
| --------------------------- | ------------------------------------------------------------------------------------------------------ |
| python-engineer             | Builds FastAPI/Django features end-to-end (model, service, endpoint, tests); debugs tracebacks, logs, Celery errors, pytest failures |

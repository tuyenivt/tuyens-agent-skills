...
- [ ] **Streaming for unbounded reads** (export endpoints, bulk exporters, scheduled jobs): use `await session.stream_scalars(select(X))` or `result.yield_per(N)` over `.all()`; combined with `StreamingResponse` (FastAPI) the response starts before the full result set is materialized in memory. `.all()` on a 100k-row result holds the whole set in RAM and stalls the event loop while it builds.
- [ ] **Pagination count cost**: total-count pagination (`Paginator(qs, n).count()` Django, `select(func.count()).select_from(...)` SQLAlchemy) issues a separate `COUNT(*)` on every page request - on a 50M-row table this is 200ms-2s _per page_ even with proper indexes, because `COUNT(*)` always scans. Prefer **keyset pagination** (`WHERE id > :last_id ORDER BY id LIMIT N`) when the UI doesn't require a total page count, or cache the count for short windows when it does.

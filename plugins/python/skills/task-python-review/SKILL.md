...
- [ ] **Idempotency on state-mutating writes**: `POST` / `PUT` / `PATCH` endpoints that create or charge (orders, payments, refunds, subscriptions) accept an `Idempotency-Key` header (or equivalent business key) and dedupe via DB unique constraint or a dedicated idempotency table. Without it, a client retry on a 504 issued from the LB after the row was committed produces a duplicate. Note that "Celery task is idempotent" is a separate concern (worker-side); HTTP idempotency lives at the endpoint
- [ ] **Response model excludes server-internal fields**: `OrderResponse` / DRF serializer must not include `internal_notes`, `audit_log`, `is_test`, `password_hash`, `mfa_secret`, `last_login_ip`, etc. - even when the ORM is not returned directly, a Pydantic response model with `from_attributes=True` happily reads internal columns when their names match. Compare the response schema fields against the ORM column list explicitly
